...
test(
	'Filters the related assets table and highlights the cell when a matrix cell is clicked',
	{tag: ['@LPD-93351']},
	async ({apiHelpers, page, projectPage, projectsPage}) => {
		...
		const decisionMakerConsiderationAsset = getRandomString();
		...
		try {
			const {
				...
				await test.step('Resolve the persona and funnel-stage category ids', async () => {
					...
					return {
						...
						endUserId: await getTaxonomyCategoryId(
							apiHelpers,
							siteId,
							'Personas',
							'End User'
						),
						retentionId: await getTaxonomyCategoryId(
							apiHelpers,
							siteId,
							'Funnel Stage',
							'Retention'
				...
				await apiHelpers.objectEntry.postObjectEntry(
					{
						keywords: [taskTag],
						objectEntryFolderExternalReferenceCode: 'L_CONTENTS',
						taxonomyCategoryIds: [decisionMakerId, considerationId],
						title: decisionMakerConsiderationAsset,
					},
					'cms/basic-web-contents',
					space.name
				);
			...
			await test.step('Both related assets are listed initially', async () => {
				await expect(
					dataSetPage.assetLink(championAwarenessAsset)
				).toBeVisible();
				await expect(
					dataSetPage.assetLink(decisionMakerConsiderationAsset)
				).toBeVisible();
			});

			await test.step('Clicking a cell filters the table and highlights the cell', async () => {
