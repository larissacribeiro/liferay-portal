...
test('table is updated when changing page/pagination number', async ({
	apiHelpers,
	page,
	...
	for (let i = 1; i <= 20; i++) {
		expect(
			page.getByRole('cell', {exact: true, name: `${i}`})
		).toBeVisible();
	}

	await expect(
		page.getByRole('cell', {name: '21', exact: true})
	).not.toBeVisible();

	await page.getByLabel('Go to page, 2').click();

	expect(page.getByRole('cell', {name: '21'})).toBeVisible();
	...
	expect(page.getByRole('cell', {name: '21', exact: true})).toBeVisible();
});
